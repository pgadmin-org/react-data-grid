...
function RangeSelection() {
  ...
  function handleExportToCsv() {
    ...
    exportToCsv(gridRef.current!.element!, 'RangeSelection.csv');
  ...
  async function handleExportToPdf() {
    ...
    await exportToPdf(gridRef.current!.element!, 'RangeSelection.pdf');
  ...
  function handleMultiCopy({ sourceRows, sourceColumnKeys }: MultiCopyEvent<Row>): void {
    if (window.isSecureContext) {
      ...
      for (const row of sourceRows) {
        rows.push(sourceColumnKeys.map((c) => (row as any)[c]).join('\t'));
      }
  ...
  function handleMultiPaste(pasteEvent: MultiPasteEvent) {
    ...
    for (let i = 0; i <= getRangeSize(sourceRange.startRowIdx, sourceRange.endRowIdx); i++) {
      for (
        ...
        (newRows[destinationStartRow + i] as any)[destinationColumnKey] = (
          newRows[sourceStartRow + i] as any
        )[sourceColumnKey];
      }
    }
  ...
  return (
    <>
      ...
      <DataGrid
        ...
        className={`fill-grid·${gridClassname}`}
        direction={direction}
